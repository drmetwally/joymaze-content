...
const formatHookFact = (hookFact) => {
  const trimmed = String(hookFact || '').trim();
  if (!trimmed) {
    return '';
  }
...
export const AnimalHookScene = ({
  ...
  const displayText = formatHookFact(hookFact);
  const words = displayText ? displayText.split(/\s+/).filter(Boolean) : [];
