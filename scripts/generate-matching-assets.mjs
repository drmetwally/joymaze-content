...
async function main() {
  ...
  // Each pair i occupies column i in both rows: [i, 6+i]
  const pairs = pairSlice.map((label, i) => ({
    id: i,
    label,
    positions: [i, 6 + i],  // row0[i], row1[i]
    centerA: { x: i, y: 0 },
    centerB: { x: i, y: 1 },
  }));

  // pairOrder: top-to-bottom (all columns have same yMidpoint = 0.5, stable sort by id)
