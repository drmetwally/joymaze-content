...
function buildImagenPrompt(prompt) {
  ...
  const sideCharacterGuard = 'The protagonist is the ONLY animal subject visible in this scene unless the prompt explicitly names and describes a second character. Do not render any other animal, bird, insect, or creature as a foreground or mid-ground subject. Background sounds and ambience — birds chirping, crickets, distant wildlife — are environmental texture only and must NOT appear as visible creatures in the image. Never invent a companion, bystander, human, child, or additional animal that the prompt does not explicitly describe.';
