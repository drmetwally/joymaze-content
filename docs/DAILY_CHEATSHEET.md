...
### 2B — Activity Puzzle Images

**Auto-generated by `npm run daily` (from activity manifest):**
- Maze → `output/raw/maze/`
- Word Search → `output/raw/wordsearch/`

**Engine-generated (run once per day — theme auto-picked from brief):**

```bash
# Coloring page via Imagen → branded post
npm run coloring:generate:imagen
npm run puzzlepost:generate -- --type coloring --slug <generated-slug>
...
- Find-Diff — engine planned; visual art pipeline (Imagen scene + sticker patches) not yet built

**Manual puzzle-post commands (one-off or manifest-driven):**
```bash
...
## Addendum — KDP Batch Puzzle Collections

> **Not part of daily workflow.** Run ad-hoc when building Amazon KDP activity book content.

Generates N maze + N wordsearch puzzles for a theme, outputs only `puzzle.png` per puzzle into one collection folder.

```bash
# Default: 15 word searches + 15 mazes, theme "monster truck"
npm run batch:puzzles

# Custom theme + count
npm run batch:puzzles -- --theme "ocean animals" --count 10

# Dry-run (shows commands without generating)
npm run batch:puzzles:dry -- --theme "space adventure"
```

Output: `output/puzzle-collection/{theme-slug}/ws-01.png … ws-15.png + maze-01.png … maze-15.png`

Flags: `--theme`, `--count` (max 15), `--ws-difficulty` (default: medium), `--maze-difficulty` (default: hard)
